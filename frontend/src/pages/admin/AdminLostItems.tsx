...
import { getLostItems, updateItemStatus, deleteItem } from '@/services/adminApi';
import { 
  Search, 
  Filter, 
  MoreHorizontal, 
...
const AdminLostItems = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      const response = await getLostItems(params);
      if (response.success) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Lost Items Management</h1>
        <p className="text-gray-600">Manage and monitor all lost item reports</p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Lost Items ({items.length})</CardTitle>
          <CardDescription>
            A list of all lost item reports in the system
          </CardDescription>
        </CardHeader>
        <CardContent>
